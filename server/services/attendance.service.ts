...
export async function markAttendance(
  ...
  await writeAuditLog({
    ...
    metadata: { roundParticipantId, roundId: participant.roundId },
  });

  // Tell the student what was recorded, so a wrong mark can be challenged.
  const marked = attendanceRecord as any;
  const app = marked?.roundParticipant?.application;
  if (app?.student?.id) {
    await PlacementNotifications.attendanceMarked(
      app.student.id,
      participant.round.title,
      app.jobRole?.title ?? "the drive",
      data.status
    );
  }
...
export async function bulkMarkAttendance(
  ...
  await writeAuditLog({
    ...
    entity: "AttendanceRecord",
    metadata: {
      operation: "bulkMarkAttendance",
      updated,
      failed: failed.length,
      records: data.records.map((r) => ({
        roundParticipantId: r.roundParticipantId,
        status: r.status,
      })),
    },
  });
