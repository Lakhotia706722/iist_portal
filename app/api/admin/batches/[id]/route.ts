...
import { updateBatch, deleteBatch } from "@/server/services/batch.service";
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    return Response.json(await updateBatch(params.id, parsed.data, actor.id));
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    const actor = await requirePermission("batch:write");
    await deleteBatch(params.id, actor.id);
