import { NextRequest, NextResponse } from "next/server";
import { requirePermission } from "@/lib/rbac/server-guard";
import { extractRequestMeta } from "@/server/services/audit.service";
import { getTestById, updateTest } from "@/server/services/skillup.service";
import { updateTestSchema } from "@/lib/validations/skillup";
import { handleApiError } from "@/lib/api-utils";

interface RouteParams {
  params: { id: string };
}

export async function GET(_request: NextRequest, { params }: RouteParams) {
  try {
    await requirePermission("skillup:read:all");
    return NextResponse.json({ test: await getTestById(params.id) });
  } catch (error) {
    return handleApiError(error);
  }
}

export async function PATCH(request: NextRequest, { params }: RouteParams) {
  try {
    const user = await requirePermission("skillup:write");
    const data = updateTestSchema.parse(await request.json());
    const test = await updateTest(
      params.id,
      data,
      user.id as string,
      extractRequestMeta(request)
    );
    return NextResponse.json({ message: "Test updated", test });
  } catch (error) {
    return handleApiError(error);
  }
}
